function S = segment_2D (I, opts)
    ...
    S.seg = S.seg.*repmat(I(bw),[1 length(i1)]);
    ...
    %{
    %distances between the seeds; A seed influences a pixel if it's closer
    %to that pixel than it is to another seed that influences the pixel
    seed_seed = nan(length(i1)); %seed-to-seed influence
    for seed = 1:length(i1)
        nearby = D(:,seed)<nh_size*sqrt(2);
        nearby(1:seed) = false;
        seed_seed(seed,nearby) = im_influence(i1(seed),i2(seed),nearby);
    end
    
    seed_seed = max(seed_seed, seed_seed');

    %for each pixel, identify its weightings
    [Y,X] = find(bw);
    im_weights = zeros(size(im_influence)); %this will be the final weighting image
    for pixel = 1:length(X)
        [~, seed_ixs] = sort(squeeze(im_influence(Y(pixel),X(pixel),:)));
        seed_ixs = flipud(seed_ixs(weights>0));
        
        if isempty(seed_ixs)
            %this is an error
            error('A pixel was not assigned to a seed.')
        end

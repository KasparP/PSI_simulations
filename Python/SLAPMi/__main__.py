...
#    Fk_nuc = prox_matrix(Fk-U_Fk_nuc,lambdaFk_nuc/rho,prox_l1)
	...
	for it in tidx2:
		PSu = Pu(it).dot(Su)
		PSk = Pk(it).dot(Sk)
		Fu[:,it] = sp.linalg.solve( PSu.T.dot(PSu)+2*sp.eye(Nsu) , (PSu.T.dot(U_X_PSFk[:,it]) + (U_Fu_nuc[:,it]+U_Fu_nuc[:,it]) + (Fu_nuc[:,it]+Fu_nn[:,it])))
		Fk[:,it] = sp.linalg.solve( PSk.T.dot(PSk)+2*sp.eye(Nsk) , (PSk.T.dot(U_X_PSFu[:,it]) + (U_Fk_nuc[:,it]+U_Fk_nuc[:,it]) + (Fk_nuc[:,it]+Fk_nn[:,it])))
		print '\b'*8
		print "'%08d'" %it
...
def lossfun_aug():
		...
		l_Poiss = Y[:,tidx2]*(np.log(Y[:,tidx2])-log(X[:,tidx2]))
		...
		l_Fk = l_Fk + (rho/2)*sum((np.flatten(Fk_nuc)-np.flatten(Fk)+np.square(np.flatten(U_Fk_nuc))-np.square(np.flatten(U_Fk_nuc))))
		l_Fk = l_Fk + (rho/2)*sum((np.flatten(Fk_nn)- np.flatten(Fk)+np.square(np.flatten(U_Fk_nn)) -np.square(np.flatten(U_Fk_nn))))
		...
		l_Fu = l_Fu + (rho/2)*sum((np.flatten(Fu_nuc)-np.flatten(Fu)+np.square(np.flatten(U_Fu_nuc))-np.square(np.flatten(U_Fu_nuc))))
		l_Fu = l_Fu + (rho/2)*sum((np.flatten(Fu_nn)-np.flatten(Fu)+np.square(np.flatten(U_Fu_nn))-np.square(np.flatten(U_Fu_nn))))

		l_Sk = 0
		l_Sk = l_Sk + (rho/2)*sum((np.flatten(Sk_nn)-np.flatten(Sk)+np.square(np.flatten(U_Sk_nn))-np.square(np.flatten(U_Sk_nn))))
		l_Su = 0
		l_Su = l_Su + (rho/2)*sum((np.flatten(Su_nn)-np.flatten(Su)+np.square(np.flatten(U_Su_nn))-np.square(np.flatten(U_Su_nn))))
